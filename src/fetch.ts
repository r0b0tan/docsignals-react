...
export async function fetchHtml(url: string): Promise<FetchResult> {
  ...
  try {
    ...
    // Endpoint is configured via VITE_PROXY_URL; falls back to Vite middleware in dev
    const proxyEndpoint = import.meta.env.VITE_PROXY_URL
      || (import.meta.env.DEV ? '/proxy' : 'https://docsignals-proxy.wutzewatz.workers.dev');
